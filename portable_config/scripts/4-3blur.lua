...
-- boxblur値とoverlay_w除数を script-opts/4-3blur.conf から読み込む対応
--
-- 2025-12 改善点:
...
local function read_blur_conf()
    local conf_path = mp.command_native({"expand-path", "~~/script-opts/4-3blur.conf"})
...
local function build_lavfi()
    ...
    if src_w == 720 and src_h == 576 then
        ...
        local center44_w = math.floor(overlay_w * 0.44 + 0.5)
...
[tmp][blurRout]overlay=x=%d:y=0,setdar=16/9[vo]
    ...
    if src_h == 720 and src_w >= 1130 and src_w <= 1150 then
        ...
        local blur_w = math.floor(target_w / 7.3 + 0.5) -- 元の指定を維持
        local blur_h = src_h

        local center44_w = math.floor(target_w * 0.44 + 0.5)
...
[tmp][blurRout]overlay=x=%d:y=0[vo]
    ...
    if force_scale then
        ...
        local center44_w = math.floor(scaled_w * 0.44 + 0.5)
...
local function apply_overlay()
    ...
    if lock_exists() then
        mp.osd_message("他のオーバーレイが (" .. (read_lockfile() or "不明") .. ") 有効中")
        return
    end
...
mp.add_key_binding("Alt+O", "4-3blur", function()
