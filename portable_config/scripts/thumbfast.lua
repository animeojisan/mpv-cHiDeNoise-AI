...
ドキュメント_ thumbfast.conf

複数のOSCスクリプトに対応する新しいサムネイルエンジン

利用可能なショートカットの例（input.confに記述する）：

 <KEY>   script-binding thumbfast/thumb_rerun    # サムネイルの取得を再開する（スタックしたサムネイルを手動で修正するために使用できる）
 <KEY>   script-binding thumbfast/thumb_toggle   # サムネイルプレビューのオン／オフ
 <KEY>   script-message thumb_hwdec toggle       # サムネイルのハードデコードの有効/無効（{toggle}パラメータを指定されたデコードAPIに置き換えることができます。）
...
local options = {
    ...
    frequency = 0.125,
    -- ここから追加
    disable_on_disc_media = true, -- DVD/BD再生時にサムネイル機能を自動無効化する
    -- ここまで追加
...
if options.load == false then
    mp.msg.info("スクリプトは初期状態では無効になっています")
    return
end
-- 原因：--load-osd-console リネーム --load-console
local min_major = 0
local min_minor = 40
local min_patch = 0
...
if incompat_check(mpv_ver_curr, min_major, min_minor, min_patch) then
    mp.msg.warn("現在のmpvバージョン (" .. (mpv_ver_curr or "unknown") .. ") 低下 " .. min_major .. "." .. min_minor .. "." .. min_patch .. "，サムネイル機能は終了しました。")
    return
end
        ...
        function(success, result)
            if spawn_waiting and (success == false or (result.status ~= 0 and result.status ~= -2)) then
                ...
                if not spawn_working then -- notify users of required configuration
                    mp.commandv("show-text", "thumbfast サブプロセス作成の失敗！", 5)
                end
...
local function file_load()
    ...
    -- ここから修正
    if options.disable_on_disc_media then
        local path = mp.get_property("stream-open-filename", "")
        if path:match("^dvd://") or path:match("^bd://") then -- BDも考慮に入れる場合はbd://も追加
            auto_run = false
            mp.osd_message("　", 2)
            mp.msg.info("DVD/BD playback detected, thumbnail function disabled.")
        else
            auto_run = true -- それ以外の場合は有効にする
        end
    else
        auto_run = true -- オプションが無効な場合は常に有効
    end
    -- ここまで修正
...
mp.add_key_binding(nil, "thumb_rerun", function()
    ...
    auto_run = true
    file_load()
    mp.osd_message("サムネイル機能を再開しました", 2)
    mp.msg.info("サムネイル機能を再開しました")
end)
mp.add_key_binding(nil, "thumb_toggle", function()
    if auto_run then
        ...
        shutdown()
        file_load()
        mp.osd_message("サムネイル機能が一時的に無効になりました", 2)
        mp.msg.info("サムネイル機能が一時的に無効になりました")
    else
        auto_run = true
        file_load()
        mp.osd_message("サムネイル機能が一時的に有効になりました", 2)
        mp.msg.info("サムネイル機能が一時的に有効になりました")
    end
end)
mp.register_script_message("thumb_hwdec", function(hwdec_api)
    ...
    mp.osd_message("サムネイルの優先デコードAPIが変更されました：" .. hwdec_api, 2)
    mp.msg.info("サムネイルの優先デコードAPIが変更されました：" .. hwdec_api)
    clear()
    shutdown()
    file_load()
end)
