# Consolidated single-file edition for mpv usage.
# Based on pifroggi/vs_temporalfix:
# https://github.com/pifroggi/vs_temporalfix
#
# Original script by pifroggi
# (also known as tepete on the "Enhance Everything!" Discord Server).
#
# This file is a modified/consolidated version for mpv-oriented usage.
# Local changes include:
# - original core embedded below
# - lightweight wrapper options integrated into the public vs_temporalfix() entry point
# - compatibility shim support so temporalfix_Lite.vpy can remain unchanged
# - mpv-oriented entry point
# - fast_scale / y_only / diff_mode options
#
# Public API:
#   from mpv_temporalfix import vs_temporalfix
...
def _vs_temporalfix_original(clip, strength=400, tr=6, denoise=False, exclude=None, debug=False):
...
def _to_yuv420p16(clip: vs.VideoNode) -> vs.VideoNode:
    if clip.format is None:
        raise ValueError("Variable format clip is not supported")
    if clip.format.color_family == vs.RGB:
        return core.resize.Bilinear(clip, format=vs.YUV420P16, matrix_s="709")
    if clip.format.id != vs.YUV420P16:
        return core.resize.Bilinear(clip, format=vs.YUV420P16)
    return clip


def _round_even(v: float, minimum: int = 16) -> int:
    iv = int(round(v))
    if iv < minimum:
        iv = minimum
    if iv % 2:
...
def vs_temporalfix(
    ...
    debug: bool = False,
    *,
    fast_scale: float = 0.75,
    y_only: bool = True,
    diff_mode: bool = True,
    restore_chroma: bool = True,
    sharpen_restore: float = 0.0,
) -> vs.VideoNode:
    """
    Consolidated wrapper + original implementation in one file.

    Compatibility goals:
    - existing `from vs_temporalfix import vs_temporalfix` usage continues to work
    - `temporalfix_Lite.vpy` can stay unchanged
    - the original full-quality path is preserved internally

    Added options:
        fast_scale: 0.50-1.00 typical. Lower is faster, weaker.
        y_only: process luma only for lower cost.
        diff_mode: run the original algorithm on a downscaled guide and merge the temporal diff back.
        restore_chroma: when y_only=True, keep original U/V.
        sharpen_restore: optional tiny merge towards original luma (0.0-0.15 typical).
    """

    if not isinstance(clip, vs.VideoNode):
        raise TypeError("vs_temporalfix: This is not a vapoursynth clip.")
    if not (0.25 <= fast_scale <= 1.0):
        raise ValueError("fast_scale must be between 0.25 and 1.0")

    src = _to_yuv420p16(clip)

    # Original full-quality path.
    if fast_scale >= 0.999 and not y_only and not diff_mode:
        return _vs_temporalfix_original(src, strength=strength, tr=tr, denoise=denoise, exclude=exclude, debug=debug)

    work = src
    if y_only:
        work = core.std.ShufflePlanes(src, 0, vs.GRAY)

    dw = _round_even(work.width * fast_scale)
    dh = _round_even(work.height * fast_scale)
    guide = work if (dw == work.width and dh == work.height) else core.resize.Bilinear(work, width=dw, height=dh)

    fixed_guide = _vs_temporalfix_original(
        guide,
        strength=strength,
        tr=tr,
        denoise=denoise,
    ...
    if debug:
        if fixed_guide.width != work.width or fixed_guide.height != work.height:
            fixed_guide = core.resize.Bilinear(fixed_guide, width=work.width, height=work.height)
        return fixed_guide if y_only else fixed_guide

    fixed = fixed_guide if (fixed_guide.width == work.width and fixed_guide.height == work.height) else core.resize.Bilinear(
        fixed_guide, width=work.width, height=work.height
    )

    if diff_mode:
        guide_match = guide if (guide.width == fixed.width and guide.height == fixed.height) else core.resize.Bilinear(
            guide, width=fixed.width, height=fixed.height
        )
        diff = core.std.MakeDiff(fixed, guide_match)
        if diff.width != work.width or diff.height != work.height:
            diff = core.resize.Bilinear(diff, width=work.width, height=work.height)
        out_work = core.std.MergeDiff(work, diff)
    else:
        out_work = fixed

    if sharpen_restore > 0.0:
        out_work = core.std.Merge(out_work, work, weight=[min(max(sharpen_restore, 0.0), 1.0)])

    if y_only and restore_chroma:
        return core.std.ShufflePlanes([out_work, src], [0, 1, 2], vs.YUV)

    return out_work



def mpv_temporalfix(*args, **kwargs):
    """Alias for vs_temporalfix for cleaner mpv-side imports."""
    return vs_temporalfix(*args, **kwargs)
